...
class PlayerService:
    ...
    def get_by_user(db, user_id):
        return db.scalar(select(Player).where(Player.user_id == user_id, Player.is_deleted.is_(False)))
    ...
    def create(db, user_id, data: PlayerCreate):
        ...
        # Reuse a previously deleted profile so the unique user_id constraint is respected.
        deleted = db.scalar(select(Player).where(Player.user_id == user_id, Player.is_deleted.is_(True)))
        if deleted:
            for key, value in data.model_dump().items():
                setattr(deleted, key, value)
            deleted.is_deleted = False
            db.commit(); db.refresh(deleted)
            return PlayerService._response(deleted, user)
    ...
    def delete(db, user_id):
        player = db.scalar(select(Player).where(Player.user_id == user_id, Player.is_deleted.is_(False)))
        if not player:
            raise ValueError("Player profile not found.")

        # Keep the Player row because match scorecards and deliveries reference it.
        # Removing the profile from active squads makes it unavailable for new matches.
        db.execute(delete(TeamMember).where(TeamMember.player_id == player.id))
        player.is_deleted = True
        db.commit()

    @staticmethod
    def get(db, player_id):
        row = db.execute(select(Player, User).join(User, User.id == Player.user_id).where(Player.id == player_id, Player.is_deleted.is_(False))).first()
        return None if not row else PlayerService._response(row[0], row[1])

    @staticmethod
    def search(db, query=None):
        stmt = select(Player, User).join(User, User.id == Player.user_id).where(Player.is_deleted.is_(False)).order_by(Player.display_name.asc())
        if query:
