...
export default function EditTeam() {
  ...
  function useUpdatedTeam(updated: Team, note: string) { setTeam(updated); setMessage(note); setError(""); }

  async function saveDetails(event: React.FormEvent) {
    event.preventDefault(); if (!team) return;
    setSaving(true); setMessage(""); setError("");
    try {
      const updated = await teamsApi.update(team.id, { name: form.name.trim(), short_name: form.short_name.trim().toUpperCase(), city: form.city.trim() || null, country: form.country.trim() || null, logo_url: form.logo_url.trim() || null, description: form.description.trim() || null });
  ...
  return <main className="create-page"><div className="create-container">
    ...
    <div className="create-layout">
      ...
      <section className="form-card player-picker"><div className="form-card-heading"><div><p className="directory-kicker">SQUAD</p><h2>Manage players</h2></div><Users size={22} /></div>
        ...
        <div className="picker-footer"><span>{team.members.length <= 5 ? "Keep at least five players in a team." : "Remove players who are no longer in the squad."}</span><strong>{team.members.length}/11</strong></div>
