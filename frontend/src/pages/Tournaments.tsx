...
type TournamentAwards = Awaited<
  ReturnType<typeof tournamentsApi.awards>
>;
...
export default function Tournaments() {
  ...
  const [seriesAwards, setSeriesAwards] = useState<
    Record<number, TournamentAwards>
  >({});
    ...
    Promise.all([tournamentsApi.list(), teamsApi.list()])
      .then(async ([ts, tsTeams]) => {
        ...
        const results = await Promise.allSettled(
          completed.map((t) => tournamentsApi.awards(t.id))
        );

        const next: Record<number, TournamentAwards> = {};

        results.forEach((r, i) => {
          if (r.status === "fulfilled") {
            next[completed[i].id] = r.value;
          }
        });

        setSeriesAwards(next);
      })
      .catch(() => setToast("Unable to load tournaments."))
  ...
  async function submit(e: FormEvent) {
    ...
    if (form.team_ids.length < 2) {
      return setToast("Select at least two teams.");
    }
  ...
  return (
    <main className="directory-page tournament-page">
      <style>{`
        .tournament-series-award{
          margin-top:14px;
          padding:12px 13px;
          border-radius:14px;
          background:linear-gradient(135deg,#fff8e8,#f8f0df);
          border:1px solid rgba(216,167,70,.3)
        }

        .tournament-series-award span{
          display:block;
          color:#9a7022;
          font-size:9px;
          font-weight:900;
        ...
        .tournament-series-award strong{
          display:block;
          margin-top:4px;
          color:#10161d;
          font-size:15px;
      ...
      <div className="directory-container">
        <section className="directory-hero tournament-hero">
          <div className="directory-hero-copy">
            <span className="directory-kicker">
              <Trophy size={14} /> CRICKET TOURNAMENTS
            </span>

            <h1>
              Compete.
              <br />
              <span>Track. Climb.</span>
            </h1>

            <p>
              Follow tournaments, fixtures, team standings and cricket
              stories in one place.
            </p>

            {isAuthenticated && (
              <button
                className="directory-primary-action"
        ...
        {loading ? (
          ...
          <div className="directory-empty">
            ...
            <p>
              Create the first tournament using your registered teams.
            </p>
          </div>
        ) : (
            ...
            {visible.map((t) => (
              ...
              >
                <div className="tournament-card-top">
                  <span
                    className={`match-status-pill tournament-status-${t.status.toLowerCase()}`}
                  >
                    {t.status}
                  </span>
                ...
                <div className="tournament-team-row">
                ...
                {t.status === "COMPLETED" &&
                  seriesAwards[t.id]?.man_of_the_series && (
                    <div className="tournament-series-award">
                      <span>MAN OF THE SERIES</span>

                      <strong>
                        {
                          seriesAwards[t.id]
                            .man_of_the_series?.player_name
                        }
                      </strong>
      ...
      {open && (
        ...
        >
          <form
            className="cp-modal tournament-create-modal"
            onSubmit={submit}
          ...
          >
            ...
            <div className="tournament-team-picker">
              <div className="tournament-picker-head">
                <strong>Select teams</strong>
                <small>
                  {form.team_ids.length} selected
                </small>
              </div>

              <div className="tournament-team-options">
                {teams.map((team) => (
                  <button
                    type="button"
                    key={team.id}
